...
public class PeopleController {
  ...
  public ResponseEntity<Map<String, List<Person>>> groupPeopleDeveloperInCity() {
    return ResponseEntity.ok().body(personRepo.groupPeopleDeveloperInCity());
  }
  //1.2 Tìm ra thành phố có độ tuổi trung bình của lập trình viên trẻ nhất
  @GetMapping("youngestdevs")
  public ResponseEntity<List<Map.Entry<String,Double>>> minCitiesHasAveAgeForDeveloper() {
    return ResponseEntity.ok().body(personRepo.minCitiesHasAveAgeForDeveloper());
  }
  //1.3 Liệt kê tỷ lệ nam/nữ ở từng thành phố
  ...
  //1.4 Tính mức lương trung bình của tất cả những người trên 30 tuổi
...
//    return ResponseEntity.ok().body(personRepo.maleFemaleRatio(entry.getValue()));
//  }
}
